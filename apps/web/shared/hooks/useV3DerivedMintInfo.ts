...
export const useV3DerivedMintInfo = ({
  ...
  const { typedValue, independentField, dependentField } = useLiquidityAmountsStore();
  ...
  // моковий пул — коли пулу нема, але ціна задана й валідна
  const mockPool = useMemo(() => {
    if (tokenA && tokenB && tier && price && !invalidPrice) {
      const currentTick = priceToClosestTick(price);
      const currentSqrt = TickMath.getSqrtRatioAtTick(currentTick);
      return new Pool(tokenA, tokenB, tier, currentSqrt, BIG_INT_ZERO, currentTick, []);
    }
    return undefined;
  ...
  }, [position, dependentField, tokenA, tokenB]);

  // дозволи на single-sided депозит

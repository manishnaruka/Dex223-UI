...
import { FullMath } from "@/sdk_bi/utils/fullMath";

import { ONE, Q96, ZERO } from "../internalConstants";

const MaxUint160 = BigInt(2) ** BigInt(160) - ONE;

function multiplyIn256(x: bigint, y: bigint): bigint {
  const product = x * y;
  return product & MaxUint256; // Bitwise AND with MaxUint256
}

function addIn256(x: bigint, y: bigint): bigint {
  const sum = x + y;
  return sum & MaxUint256; // Bitwise AND with MaxUint256
}
...
export abstract class SqrtPriceMath {
  ...
  private static getNextSqrtPriceFromAmount1RoundingDown(
    ...
    if (add) {
      const quotient =
        amount <= MaxUint160 ? (amount << BigInt(96)) / liquidity : (amount * Q96) / liquidity;

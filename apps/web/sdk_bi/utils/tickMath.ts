...
import { MaxUint256 } from "@/sdk_bi/constants";
import { mostSignificantBit } from "@/sdk_bi/utils/mostSignificantBit";

import { ONE, ZERO } from "../internalConstants";

function mulShift(val: bigint, mulBy: string | number) {
...
export abstract class TickMath {
   ...
   */
  public static MIN_SQRT_RATIO = BigInt("4295128739");
  /**
   * The sqrt ratio corresponding to the maximum tick that could be used on any pool.
   */
  public static MAX_SQRT_RATIO = BigInt("1461446703485210103287273052203988822378723970342");
   ...
   */
  public static getTickAtSqrtRatio(sqrtRatioX96: bigint) {
    invariant(
    ...
    let r;
    if (msb >= 128) {
      // If msb >= 128, perform a signed right shift
      r = sqrtRatioX128 >> BigInt(msb - 127);
    } else {
      // If msb < 128, perform a left shift. If msb is negative, it will behave similarly to unsigned left shift.
      r = sqrtRatioX128 * BigInt(2) ** BigInt(127 - msb);
    }

    let log_2 = (BigInt(msb) - 128n) * BigInt(2) ** 64n;

    for (let i = 0; i < 14; i++) {
      r = (r * r) >> 127n; // Signed right shift after multiplication
      const f: bigint = r >> 128n; // Get the high bits
      log_2 |= f << BigInt(63 - i); // Perform the left shift for log_2 calculation
      r = r >> f; // Right shift by f
    }

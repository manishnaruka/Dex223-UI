...
export default function TransactionInfoCard({ transaction }: TransactionInfoCardProps) {
    return (
        <div className="bg-secondary-bg rounded-3 p-5 space-y-4">
            ...
            <div className="flex justify-between items-center">
                ...
                <span className={`text-12 px-3 py-1 rounded-2 font-medium ${
                    transaction.status === 'executed' ? 'bg-green text-black' :
                    transaction.status === 'approved' ? 'bg-blue text-white' :
                    transaction.status === 'declined' ? 'bg-red text-white' :

...
import { Address, formatUnits, decodeFunctionData } from "viem";
...
export default function useMultisigTransactions() {
  ...
  const [unknownTokenCache, setUnknownTokenCache] = useState<Map<string, { symbol: string; decimals: number }>>(new Map());
  ...
  const formatTransaction = useCallback(async (

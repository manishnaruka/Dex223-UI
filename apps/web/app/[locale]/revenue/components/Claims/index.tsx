...
import { ButtonColor, ButtonVariant as ButtonVariantType, ButtonSize } from "@/components/buttons/Button";
import { Standard } from "@/sdk_bi/standard";
import Checkbox from "@repo/ui/checkbox";
import IconButton, { IconButtonVariant, IconButtonSize } from "@/components/buttons/IconButton";
        ...
        <>
            ...
            <div className="hidden lg:block pr-5 pl-5 pb-5 rounded-3 overflow-hidden bg-table-gradient">
                <div className="grid grid-cols-[minmax(200px,2.5fr),_minmax(200px,2fr),_minmax(150px,1.2fr),_minmax(150px,1.2fr),_minmax(120px,1fr)] relative border-b border-quaternary-bg">
                    ...
                    <div className="text-tertiary-text text-13 h-[60px] flex items-center justify-end pr-4">Amount in USD</div>
                    <div className="text-tertiary-text text-13 h-[60px] flex items-center justify-center">Action</div>
                </div>

                {isLoading ? (
                    <SkeletonTheme baseColor="#272727" highlightColor="#2E2F2F" borderRadius="0.5rem" enableAnimation={false}>
                        {[...Array(8)].map((_, index) => (
                            ...
                            <React.Fragment key={key}>
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <Checkbox checked={isSelected} handleChange={() => handleTokenSelect(o.id)} id={`claim-token-${o.id}`} />
                                        <Image
                                        ...
                                        <div className="flex min-w-0 justify-center gap-2 items-center">
                                            <span className="truncate text-primary-text text-14 font-medium">{o.name}</span>
                                ...
                                >
                                    <div className="flex flex-col w-full">
                                        <div className="flex items-center gap-2">
                                            <ExternalTextLink
                                                text={truncateMiddle(o.erc20Address || "", { charsFromStart: 5, charsFromEnd: 3 })}
                                                href={getExplorerLink(ExplorerLinkType.ADDRESS, o.erc20Address, o.chainId)}
                                            />
                                            <IconButton variant={IconButtonVariant.COPY} text={o.erc20Address} />
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <ExternalTextLink
                                                text={truncateMiddle(o.erc223Address || "", { charsFromStart: 5, charsFromEnd: 3 })}
                                                href={getExplorerLink(ExplorerLinkType.ADDRESS, o.erc223Address, o.chainId)}
                                            />
                                ...
                                <div
                                    className={clsx("min-h-[30px] flex items-center justify-center border-b border-quaternary-bg")}
                                >
                                    <Button
                ...
                {selectedCount > 0 && !isLoading && (
                    <div className="mt-4 p-4 bg-tertiary-bg rounded-3 flex items-center justify-between gap-4 border border-quaternary-bg">
                        <div className="flex items-center gap-4">
                            <span className="text-primary-text text-14">Total claim: {selectedCount} token{selectedCount !== 1 ? 's' : ''}</span>
                            <button
                        ...
                        <div className="flex items-center gap-6">
                            <div className="flex items-center gap-2">
                                ...
                                <Button variant={ButtonVariantType.OUTLINED} colorScheme={ButtonColor.GREEN} size={ButtonSize.SMALL}>Edit</Button>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="text-primary-text text-14">Total reward: ${totalReward.toFixed(2)}</span>
                                <Button
                                    variant={ButtonVariantType.CONTAINED}
                                    colorScheme={ButtonColor.GREEN}
            ...
            <div className="lg:hidden space-y-4">
                {isLoading ? (
                    ...
                    return (
                        <div key={key} className="bg-tertiary-bg rounded-3 p-4">
                            <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center gap-3">
                                    <Checkbox checked={isSelected} handleChange={() => handleTokenSelect(o.id)} id={`claim-token-mobile-${o.id}`} />
                                    <Image
                                        src={o.logoURI || "/images/tokens/placeholder.svg"}
                                        width={24}
                                        height={24}
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-primary-text text-14">{o.amount} {o.symbol}</span>
                                </div>
                ...
                {selectedCount > 0 && !isLoading && (
                    <div className="bg-tertiary-bg rounded-3 p-4 space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-primary-text text-14">Gas price: $12.23</span>
                            <Button variant={ButtonVariantType.CONTAINED} colorScheme={ButtonColor.GREEN} size={ButtonSize.SMALL}>Edit</Button>
                        </div>

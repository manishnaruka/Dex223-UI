...
const buttonVariantClassnameMap = {
    [ButtonVariant.CONTAINED]: {
        ...
        [ButtonColor.LIGHT_RED]: "bg-red-bg text-secondary-text border-transparent border hocus:border-red-light hocus:bg-red-bg-hover hocus:text-primary-text",
        [ButtonColor.LIGHT_YELLOW]: "bg-yellow-bg text-secondary-text border-transparent border hocus:border-yellow-light hocus:bg-yellow-bg-hover hocus:text-primary-text",
        [ButtonColor.LIGHT_GREEN]: "bg-green-bg text-secondary-text border-transparent border hocus:border-green hocus:bg-green-bg-hover hocus:text-primary-text",
    },
    [ButtonVariant.OUTLINED]: {
        [ButtonColor.RED]: "border border-primary text-secondary-text hocus:bg-red-bg hocus:border-primary-text hocus:text-primary-text",
        ...
        [ButtonColor.LIGHT_GREEN]: "bg-green-bg text-primary-text border-transparent border hocus:border-green",
    },
};
...
export default function Button({ variant = ButtonVariant.CONTAINED, size = ButtonSize.LARGE, mobileSize, tabletSize, startIcon, endIcon, fullWidth, colorScheme = ButtonColor.GREEN, children, className, isLoading, ...props }) {
    const _mobileSize = mobileSize || size;
    const _tabletSize = tabletSize || size;
    return (<button className={clsxMerge("flex items-center justify-center gap-2 duration-200 disabled:pointer-events-none", buttonVariantClassnameMap[variant][colorScheme], buttonSizeClassnameMap[size], tabletButtonSizeClassnameMap[_tabletSize], mobileButtonSizeClassnameMap[_mobileSize], fullWidth && "w-full", disabledClassnameMap[variant], isLoading && "opacity-50 pointer-events-none", className)} {...props}>
      {startIcon}
      {children}
      {endIcon}
    </button>);
